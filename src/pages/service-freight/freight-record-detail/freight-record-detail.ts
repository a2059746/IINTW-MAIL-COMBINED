...
export class FreightRecordDetailPage {
       ...
       = this._state.orderRef
        ...
        .subscribe(detail => {
          ...
          .map(snaps => snaps.map(snap => ({
            ...
            this.total.payAmount = this.total.deposit + this.total.giftsPrice;
  ...
  goBarcode(fTag_ID) {
    ...
    console.log(this._state.orderDetail.Total);
  ...
  statePay() {
    return this.stateList.find(a => a._State_ID == this._state.orderDetail._Pay_State)
  }

  stateFreight() {
    return this.stateList.find(a => a._State_ID == this._state.orderDetail._Box_State)
  }
